...
# In-memory storage: {user_id: {"count": int, "username": str}}
_scoreboard_data = defaultdict(lambda: {"count": 0, "username": ""})
    ...
    if os.path.exists(SCOREBOARD_FILE):
        try:
            with open(SCOREBOARD_FILE, 'r') as f:
                ...
                _scoreboard_data = defaultdict(lambda: {"count": 0, "username": ""}, data)
...
def get_leaderboard(limit=10):
    ...
    Returns list of tuples: (user_id, count, username) sorted by count descending
    """
    ...
    # Return top N as list of tuples: (user_id, count, username)
    return [
        (user_id, data["count"], data["username"])
